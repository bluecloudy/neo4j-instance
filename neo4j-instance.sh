...
function createDatabase {
    ...
    if [ ! -d "ports/$lastPort" ]; then
        ...
        if [ -e "${skeletonPath}/conf/neo4j.conf" ]; then
            cat "${skeletonPath}/conf/neo4j.conf" | sed -e "s/#dbms.connector.http.listen_address=:7474/dbms.connector.http.listen_address=:$lastPort/" | sed -e "s/#dbms.connector.https.listen_address=:7473/dbms.connector.https.listen_address=:$lastSslPort/" | sed -e "s/dbms.connector.bolt.enabled=false/dbms.connector.bolt.enabled=true/" > ports/$lastPort/conf/neo4j.conf
        fi
